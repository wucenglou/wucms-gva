...
const getTableData = async () => {
    const table = await getPatientList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
...
const onDelete = async () => {
    ...
    const res = await deletePatientByIds({ ids })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
...
// 删除行
const deletePatientFunc = async (row) => {
    const res = await deletePatient({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
...
const enterDialog = async () => {
    elFormRef.value?.validate(async (valid) => {
        ...
        switch (type.value) {
            case 'create':
                res = await createPatient(formData.value)
                break
            case 'update':
                res = await updatePatient(formData.value)
                break
            default:
                res = await createPatient(formData.value)
                break
