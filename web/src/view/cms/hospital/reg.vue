...
const getTableData = async () => {
    const table = await getRegList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
...
const onDelete = async () => {
    ...
    const res = await deleteRegByIds({ ids })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
...
// 更新行
const updateRegFunc = async (row) => {
    const res = await findReg({ id: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.Reg
...
// 删除行
const deleteRegFunc = async (row) => {
    const res = await deleteReg({ id: row.ID })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
...
const enterDialog = async () => {
    elFormRef.value?.validate(async (valid) => {
        ...
        switch (type.value) {
            case 'create':
                res = await createReg(formData.value)
                break
            case 'update':
                res = await updateReg(formData.value)
                break
            default:
                res = await createReg(formData.value)
                break
