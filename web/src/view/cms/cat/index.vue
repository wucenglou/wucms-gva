        ...
        <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="添加分类">
            <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
                ...
                <el-form-item label="上级栏目" :label-width="formLabelWidth">
                  <el-cascader v-model="formData.parentId" :disabled="dialogType == 'add'" :options="catOption"
                    :props="{ checkStrictly: true, label: 'name', value: 'term_id', disabled: 'disabled', emitPath: false }"
                ...
                <el-form-item label="描述:" prop="description">
                        <el-input v-model="formData.description" type="textarea" :clearable="true" placeholder="请输入" />
                    </el-form-item>
...
const getTableData = async () => {
    ...
    if (table.code === 0) {
        ...
        catOption.value = table.data.list
        // const catOptionA = table.data.list
        // catOption.value = catOptionA
...
const onDelete = async () => {
    ...
    const res = await deleteTermStructByIds({ ids })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
...
const updateTermStructFunc = async (row) => {
    const res = await findTermStruct({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
const deleteTermStructFunc = async (row) => {
    const res = await deleteTermStruct({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
            type: 'success',
            message: '删除成功'
    ...
    elFormRef.value?.validate(async (valid) => {
        ...
        switch (type.value) {
            case 'create':
                ...
                break
            case 'update':
                res = await updateTermStruct(formData.value)
                break
            default:
                res = await createTermStruct(formData.value)
                break
